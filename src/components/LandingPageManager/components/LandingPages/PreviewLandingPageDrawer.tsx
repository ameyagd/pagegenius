...
const PreviewLandingPageDrawer: React.FC<PreviewLandingPageDrawerProps> = ({ visible, onClose, pageId, onEdit, onAnalytics }) => {
    ...
    return (
        <Drawer
            ...
            extra={
                <Space>
                    <Button icon={<DesktopOutlined />} type={previewMode === 'desktop' ? 'primary' : 'default'} onClick={() => setPreviewMode('desktop')}>
                        Desktop
                    </Button>
                    <Button icon={<MobileOutlined />} type={previewMode === 'mobile' ? 'primary' : 'default'} onClick={() => setPreviewMode('mobile')}>
                        Mobile
                    </Button>
                </Space>
            }
                ...
                <>
                    <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
                        <Space>
                            ...
                            <Button icon={<BarChartOutlined />} onClick={onAnalytics}>
                                View Analytics
                            </Button>
                        </Space>
                    </div>
                    ...
                    >
                        <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Space>
                                <Text>Status:</Text>
                                <Tag color={getStatusColor(page.status)}>{page.status.replace('_', ' ')}</Tag>
                            </Space>
                            <Space>
                                <Text>Source:</Text>
                                <Tag color={page.isAiGenerated ? 'blue' : 'default'}>{page.isAiGenerated ? 'AI Generated' : 'Manual'}</Tag>
                            </Space>
                        </div>
                        ...
                        {page.imageUrl && (
                            <div style={{ marginBottom: 16 }}>
                                <Image src={page.imageUrl} alt={page.title} style={{ maxWidth: '100%' }} />
                            </div>
                        )}

                        <Paragraph style={{ whiteSpace: 'pre-line' }}>{page.content}</Paragraph>
